...
class GeneratorMinimaxBitboardMoveOrdering : public AGenerator<T, W, H> {
	public:
		GeneratorMinimaxBitboardMoveOrdering() = default;
		~GeneratorMinimaxBitboardMoveOrdering() = default;
	...
	private:
		...
		bool solve(T current, uint8_t turn) {
			...
			if (turn == 0) {
				...
				std::sort(cands.begin(), cands.begin() + n, [](const Cand& a, const Cand& b) { 
