...
class GeneratorMinimaxTTPopcount : public AGenerator<T, W, H> {
	...
	private:
		...
		inline bool getMemo(T board, uint64_t hash, uint8_t turn, bool &out) {
			...
			if (tt[idx].board != board || tt[idx].val == 0) return false;
			out = (tt[idx].val == 2);
			hits++;
			return true;
		}
		...
		inline void setMemo(T board, uint64_t hash, uint8_t turn, bool val) {
			auto &tt = (turn == 0) ? TT0 : TT1;
			size_t idx = hash & (TT_SIZE - 1);
			if (shouldReplace(tt[idx].board, board)) tt[idx] = State{board, (uint8_t)(val + 1)};
		}
			...
			if (turn == 0) {
				...
				while (anchors) {
					...
					if (!solve(next, nhash, 1)) {
						setMemo(current, hash, turn, true);
						return true;
					}
				}
			} 
