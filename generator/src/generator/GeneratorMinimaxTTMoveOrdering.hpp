...
template <Bitboard T, std::uint8_t W, std::uint8_t H, class ReplacePolicy = AlwaysReplace> class GeneratorMinimaxTTMoveOrdering : public GeneratorMinimaxTT<T, W, H, ReplacePolicy> {
	...
	bool solveOrdered(State s) {
		this->nodes++;

		T compressedBoard = compressor.compress(s.board);
		uint64_t hash = 0;
		s.board = compressedBoard;
		while (compressedBoard) {
			int b = ctzT<T>(compressedBoard);
			compressedBoard &= (compressedBoard - 1);
			hash = zobrist.hash(hash, b);
		}
		...
		if (s.turn == 0) {
			...
			std::sort(cands.begin(), cands.begin() + n, [](const Cand& x, const Cand& y){ return x.score < y.score; });
	...
	void run() override {
		...
		bool firstWins = solveOrdered(root);
